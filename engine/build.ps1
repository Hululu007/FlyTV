...
$jarOut = Join-Path $root "flytv-engine.jar"
...
if ($found) { $javac = $found.Source }
    ...
    foreach ($jdk in @("$env:USERPROFILE\.jdks\corretto-17.0.14", "$env:USERPROFILE\.jdks\ms-21.0.8", "C:\Program Files\Amazon Corretto\jdk11.0.22_7")) {
...
& $javac -encoding UTF-8 --release 11 -nowarn -cp "$libs\*" -d $out $files
